...
def model(state: State): 
    ...
    logger.info("[model] Generating content")
    ...
    logger.info(f"[model_node] Generated post:\n{geneated_linkedin_post}\n")
...
def human_node(state: State): 
    ...
    logger.info(f"[human_node] Received human feedback: {user_feedback}")
...
def end_node(state: State): 
    ...
    logger.info(f"Final Generated Post: {state["generated_post"][-1]}")
    logger.info(f"Final Human Feedback: {state["human_feedback"]}")
